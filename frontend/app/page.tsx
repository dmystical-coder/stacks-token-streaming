        ...
        try {
          ...
          if (innerValue && innerValue.value) {
            ...
            if (!stream || typeof stream !== "object") {
              console.warn(
                `Stream ${i} has invalid structure (not an object):`,
                stream
              );
              continue;
            }
            ...
            const invalidFields = requiredFields.filter(
              (field) =>
                stream[field] &&
                typeof stream[field] === "object" &&
                !("value" in stream[field])
            );
            if (invalidFields.length > 0) {
              console.warn(
                `Stream ${i} has fields without .value:`,
                invalidFields
              );
              continue;
            }
        ...
        {!isSignedIn ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <h2 className="text-2xl font-semibold mb-2">
              Welcome to Token Streaming
            </h2>
            <p className="text-zinc-500 mb-6 text-center max-w-md">
              Stream STX and SIP-010 tokens over time with vesting,
