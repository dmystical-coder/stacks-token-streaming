...
export const STREAM_ERRORS: Record<number, StreamError> = {
  100: {
    ...
    suggestion: 'Only the stream sender can cancel or pause. Only the recipient can withdraw.',
  },
  ...
  104: {
    ...
    suggestion: 'Please set a duration between 60 seconds (1 min) and 31,536,000 seconds (1 year).',
  },
  ...
  107: {
    ...
    suggestion: 'Tokens vest over time. Please wait for more tokens to vest, or check if the stream is paused.',
  },
  ...
  111: {
    ...
    suggestion: 'Please check your balance and try again. You may need more STX for gas fees.',
  },
 ...
 */
export function getErrorSuggestion(errorCode: number | bigint): string | undefined {
  const error = getStreamError(errorCode);
